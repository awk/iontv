...
+ (NSArray *)fetchProgramsWithIDS:(NSArray *)inProgramIDS inManagedObjectContext:(NSManagedObjectContext *)inMOC {
  ...
  [fetchRequest setEntity:[NSEntityDescription entityForName:@"Program" inManagedObjectContext:inMOC]];
  [fetchRequest setPredicate:[NSPredicate predicateWithFormat: @"(programID IN %@)", inProgramIDS]];

  // make sure the results are sorted as well
  [fetchRequest setSortDescriptors: [NSArray arrayWithObject:[[
    [NSSortDescriptor alloc] initWithKey:@"programID" ascending:YES] autorelease]]];
  // Execute the fetch
  NSError *error;
...
- (void)addProductionCrewWithXMLElement:(NSXMLElement *)inXMLElement {
  ...
  for (NSXMLElement *memberElement in nodes) {
    Z2ITCrewMember *aCrewMember = [NSEntityDescription insertNewObjectForEntityForName:@"CrewMember" inManagedObjectContext:[self managedObjectContext]];
    ...
    if ([memberNodes count] == 1) {
      ...
      if (!aCrewRole) {
        aCrewRole = [NSEntityDescription insertNewObjectForEntityForName:@"CrewRole"
...
- (void)addAdvisory:(NSString *)value {
  ...
  if (!newAdvisory) {
    newAdvisory = [NSEntityDescription insertNewObjectForEntityForName:@"Advisory"
                                                inManagedObjectContext:[self managedObjectContext]];
